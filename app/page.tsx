  ...
  return (
    <main className="editor-shell">
      ...
      <div className={`workspace mobile-${mobileView}`}>
        ...
        <section className="preview-panel"><div className="preview-meta"><span>Live preview</span><span>A4 · Portrait</span></div><div className="paper-wrap"><div className="paper" ref={previewRef}><div className="paper-watermark"><UniversityLogo watermark /></div><div className="paper-frame"><UniversityLogo /><h2>Bangladesh Maritime University</h2><p className="tagline">We strive for Maritime Excellence</p><p className="paper-department">{form.department || 'DEPARTMENT OF MARITIME LAW AND POLICY'}</p><h3>ASSIGNMENT ON</h3><div className="paper-fields"><p><strong>TOPIC :</strong><span>{form.topic || '\u00a0'}</span></p><p><strong>COURSE TITLE :</strong><span>{form.courseTitle || '\u00a0'}</span></p><p><strong>COURSE CODE :</strong><span>{form.courseCode || '\u00a0'}</span></p></div><div className="submission-grid"><div><h4>SUBMITTED TO -</h4><p className="submitted-block">{form.submittedTo || '\u00a0'}</p></div><div><h4>SUBMITTED BY -</h4><p><strong>NAME :</strong><span>{form.submittedByName || '\u00a0'}</span></p><p><strong>ID. :</strong><span>{form.submittedById || '\u00a0'}</span></p></div></div><p className="date-line"><strong>DATE OF SUBMISSION :</strong> {form.date ? new Date(`${form.date}T00:00:00`).toLocaleDateString('en-GB') : '\u00a0'}</p></div></div></div></section>
      </div>
