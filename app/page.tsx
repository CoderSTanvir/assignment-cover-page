  ...
  return (
    <main className="editor-shell">
      ...
      <div className={`workspace mobile-${mobileView}`}>
        ...
        <section className="preview-panel"><div className="preview-meta"><span>Live preview</span><span>A4 · Portrait</span></div><div className="paper-wrap"><div className="paper" ref={previewRef}><div className="paper-watermark"><UniversityLogo watermark /></div><div className="paper-frame"><UniversityLogo /><h2>Bangladesh Maritime University</h2><p className="tagline">We strive for Maritime Excellence</p><p className="paper-department">{form.department || 'DEPARTMENT OF MARITIME LAW AND POLICY'}</p><h3>ASSIGNMENT ON</h3><div className="paper-fields"><p><strong>TOPIC :</strong> {form.topic || '\u00a0'}</p><p><strong>COURSE TITLE :</strong> {form.courseTitle || '\u00a0'}</p><p><strong>COURSE CODE :</strong> {form.courseCode || '\u00a0'}</p></div><div className="submission-grid"><div><h4>SUBMITTED TO -</h4><p className="submitted-block">{form.submittedTo || '\u00a0'}</p></div><div><h4>SUBMITTED BY -</h4><p><strong>NAME :</strong> {form.submittedByName || '\u00a0'}</p><p><strong>ID. :</strong> {form.submittedById || '\u00a0'}</p></div></div><p className="date-line"><strong>DATE OF SUBMISSION :</strong> {form.date ? new Date(`${form.date}T00:00:00`).toLocaleDateString('en-GB') : '\u00a0'}</p></div></div></div></section>
      </div>
